...
// Disable with TALKJS_FLUTTER_INAPPWEBVIEW_LINUX_DISABLE_GL=1 to force software rendering.
bool UseGLTextureEnvOverride() {
  if (g_getenv("TALKJS_FLUTTER_INAPPWEBVIEW_LINUX_DISABLE_GL") != nullptr) {
    return false;
  }
  return true;
}
...
CustomPlatformView::CustomPlatformView(FlBinaryMessenger* messenger,
  ...
  std::string method_channel_name =
      "com.talkjs/talkjs_custom_platform_view_" + std::to_string(texture_id_);
  ...
  std::string event_channel_name =
      "com.talkjs/talkjs_custom_platform_view_" + std::to_string(texture_id_) + "_events";
